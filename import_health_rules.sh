...
### token
if ([ -z "${_token// }" ]); then
	_token=$(cat ".local.token-cmademo")
fi
...
_controller_host="$_controller_host:8090"
...
function func_import_health_rules(){
    ...
    for f in $folderPath; do 
        ...
        if [ "${healthRuleId}" == "" ]; then
            # echo "curl -X POST -H '${_auth_header}' '${_controller_url}/alerting/rest/v1/applications/${appId}/health-rules' --header 'Content-Type: application/json' --data '@${f}' ${_proxy_details}"
            # exit 1
